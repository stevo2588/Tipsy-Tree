...
public class EndRaccoonGameScreen: MonoBehaviour {
	...
	void OnGUI(){
		...
		if (winner == true){bgStyle.normal.background = happyBackdrop;}
		else bgStyle.normal.background = sadBackdrop;
		...
		//Game over message
		if (winner == true){GUI.Label (new Rect ((w_center), (h_center-(Screen.height/8)), w_center, (Screen.height/16)), "You win!");}
		else GUI.Label (new Rect ((w_center-(w_double/5)), (h_center-(Screen.height/4)), w_center, (Screen.height/16)), "You lose!");
	}
}