...
// to treat this ck as a TouchPad. A TouchPad allows the finger
...
class JoystickBoundary 
{
	var min : Vector2 = Vector2.zero;
	var max : Vector2 = Vector2.zero;
}

static private var joysticks : MyJoystick[];					// A static collection of all joysticks
...
private var guiBoundary : JoystickBoundary = JoystickBoundary();			// Boundary for joystick graphic
...
{	
	if ( !enumeratedJoysticks )
	{
		...
		joysticks = FindObjectsOfType( MyJoystick ) as MyJoystick[];
	...
	else
	{
		for(var i : int = 0;i < count; i++)
		{
			...
			if ( shouldLatchFinger && ( lastFingerId == -1 || lastFingerId != touch.fingerId ) )
			{
				...
				for ( var j : MyJoystick in joysticks )
				{
					if ( j != this )
